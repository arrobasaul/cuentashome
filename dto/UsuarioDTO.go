...
func GetUsuarios() (*[]entidades.Usuarios, error) {
	...
	for result.Next() {
		err = result.Scan(&usuario.CodUsuarios, &usuario.NombreUsuario, &usuario.Correo, &usuario.Password, &usuario.Estado)
		if err != nil {
			panic(err.Error())
		}
...
func GetUsuario(id int) (*entidades.Usuarios, *entidades.Errores) {
	...
	println(id)
	err := db.QueryRow("SELECT CodUsuarios, NombreUsuario, Correo, Password, Estado FROM Usuarios where CodUsuarios=?", id).Scan(&usuario.CodUsuarios, &usuario.NombreUsuario, &usuario.Correo, &usuario.Password, &usuario.Estado)
	if err != nil {
